      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
        ...
        <Card title="Recently Updated">
          {data?.recentlyUpdated?.length ? (
            <div className="space-y-3">
              {data.recentlyUpdated.map(cred => (
                <div key={cred.id} className="flex items-center justify-between py-2 border-b border-slate-800 last:border-0">
                  <div>
                    <p className="text-sm font-medium text-white">{cred.title}</p>
                    <p className="text-xs text-slate-500">{cred.entity?.name} · {cred.username}</p>
                  </div>
                  <span className="text-xs text-slate-500">{timeAgo(cred.updatedAt)}</span>
                </div>
              ))}
            </div>
          ) : (
        ...
        <Card title="Expiry Alerts">
          {expiryData?.length ? (
            <div className="space-y-3">
              {expiryData.map(cred => (
                <div key={cred.id} className="flex items-center justify-between py-2 border-b border-slate-800 last:border-0">
                  ...
                  <Badge variant={isExpired(cred.expiryDate) ? 'danger' : isExpiringSoon(cred.expiryDate, 7) ? 'warning' : 'info'}>
            ...
            <p className="text-slate-500 text-sm">No expiry alerts</p>
          )}
        </Card>
      </div>
    </div>
