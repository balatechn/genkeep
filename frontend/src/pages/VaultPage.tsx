...
function CredentialRow({ cred, onEdit, onDelete, isAdmin }: {
  ...
  async function handleReveal() {
    ...
    try {
      ...
      setTimeout(() => setRevealed(null), 30000); // auto-hide after 30s
    } catch { toast.error('Failed to reveal'); }
    finally { setRevealing(false); }
  }

  async function handleCopy() {
    if (revealed) { await copyToClipboard(revealed); toast.success('Copied! Clears in 20s'); return; }
    setRevealing(true);
    try {
      const r = await credentialsApi.reveal(cred.id);
  ...
  const expiredStatus = isExpired(cred.expiryDate);
  const expiringSoon = isExpiringSoon(cred.expiryDate, 7);
  ...
  return (
    <tr className="border-b border-slate-800 hover:bg-slate-800/30 transition-colors">
      ...
      <td className="py-3 px-4">
        <Badge>{cred.entity?.entityType?.label}</Badge>
      </td>
      <td className="py-3 px-4">
        {cred.urlOrIp ? (
      ...
      <td className="py-3 px-4">
        {cred.expiryDate ? (
          <Badge variant={expiredStatus ? 'danger' : expiringSoon ? 'warning' : 'success'}>
